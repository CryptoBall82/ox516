...
const NavbarIcon: React.FC<NavbarIconProps> = ({name, text, onClick, isActive}) => {
  const iconClasses = isActive ? 'text-black' : 'text-primary-foreground'; // Using theme colors
  ...
  switch (name) {
    ...
    case 'Tools':
      ...
      break;
    case 'Auto awesome': // Keep name for consistency, maps to Sparkles
  ...
  return (
    ...
    >
      ...
      <div style={{paddingRight: '5px'}}>
        <NavbarIcon name="Auto awesome" text="OfficiaX AI" onClick={navigateToOfficiaX_AIPage} isActive={true} />
      </div>
    </nav>
  );
};
